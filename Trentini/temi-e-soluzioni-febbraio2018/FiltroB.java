...
- la linea stessa
- il numero di parole di LUNGHEZZA DISPARI presenti sulla linea
- il rapporto fra il numero di parole di LUNGHEZZA PARI e quelle totali sulla linea

Stampare poi l'andamento del numero di parole di LUNGHEZZA DISPARI:
'+' se il numero cresce
'=' se è uguale
'-' se decresce
 */
...
public class FiltroB {
    ...
    throws Exception {
        ...
        while(sorgente.hasNextLine()) {
            ...
            int nr=paroleInLinea(cur,DISPARI);
            nrParole.add(nr);

            totParole.add(paroleInLinea(cur,PARI)+nr); // diverso da sopra!!!
        }

        /*
            int totParoleNelFile=0;
            for(int i: totParole){
        		totParoleNelFile+=i;
        	}
        */
        ...
        for(int l=0; l<linee.size(); l++) {
            ...
            //System.out.println((1.0*nrParole.get(l))/totParoleNelFile);
        }
